...
function appendChild(name, price, description, img){
    var child = `
        <a data-name="${name}" data-desc="${description}" data-img="${img}" class="text-decoration-none child" href="#" data-toggle="modal" data-target="#foodItemModel">
...
function appenddModalChild(name, description, img) {
    var child = `
        <div class="modal-body"><button class="close" type="button" data-dismiss="modal" aria-label="Close"><span
            ...
            <div class="modal-footer justify-content-center"><button class="btn btn-primary btn-block text-center"
                type="button">order online</button></div>
          </div>
    `;
    return child;
}
...
$('document').ready( () => {
    ...
    $(".menu-wrapper1").on('click', '.child', function () {
        ...
        $('.modal-content-menu').html(appenddModalChild(name, desc, img));
